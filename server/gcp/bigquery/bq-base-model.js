...
exports.BqBaseModel = exports.CheckUndefined = void 0;
const lo = require("lodash");
const bigquery_client_1 = require("./bigquery-client");
const bigquery_registry_1 = require("./bigquery-registry");
function CheckUndefined(obj, nestingLevel = 2) {
    if (nestingLevel === 0)
        return;
    lo.keysIn(obj).forEach((key) => {
        if (obj[key] === undefined)
            obj[key] = null;
        else if (typeof (obj[key]) === 'object')
            CheckUndefined(obj[key], (nestingLevel - 1));
    });
}
exports.CheckUndefined = CheckUndefined;
class BqBaseModel {
    constructor(rc) { }
    copyConstruct(bqItem) {
        if (bqItem == null)
            return;
        // copy all the DataStore Models Item fields to BaseBigQuery (which are present in own schema)
        let ownKeys = lo.keys(this), // own keys
        bqItemClone = lo.pick(bqItem, ownKeys); // item clone with only own fields
    ...
    getTableName(rc) {
        const registry = bigquery_registry_1.BqRegistryManager.getRegistry(this.constructor.name.toLowerCase());
    ...
    async tableExists(rc) {
        const registry = bigquery_registry_1.BqRegistryManager.getRegistry(this.constructor.name.toLowerCase());
        const dataset = await bigquery_client_1.BigQueryClient._bigQuery.dataset(registry.getDataset()), tableName = this.getTableName(rc), table = dataset.table(tableName), tableRes = await table.exists();
        return !!tableRes[0];
    }
    getTableOptions(rc) {
        if (!this.options) {
            ...
            const fields = registry.getFields().filter((field) => field.parent === undefined), recordFields = registry.getFields().filter((field) => field.parent !== undefined);
            // Supproting 2 levels of record
            for (const recField of recordFields) {
                let parentField = fields.find((fld) => fld.name === recField.parent);
                if (!parentField)
                    parentField = recordFields.find((fld) => fld.name === recField.parent);
    ...
    async init(rc) {
        ...
        rc.isAssert() && rc.assert(rc.getName(this), !lo.isEmpty(registry.getDataset()) &&
            !lo.isEmpty(registry.getTableName()) &&
            !lo.isEmpty(this.getTableOptions(rc)), 'Table properties not set');
        const dataset = await this.getDataset(rc, registry);
        const tableName = this.getTableName(rc), table = dataset.table(tableName), tableRes = await table.exists();
        if (tableRes[0]) {
            /*
            // Table metadata
    ...
    async getDataStoreTable(rc) {
        const registry = bigquery_registry_1.BqRegistryManager.getRegistry(this.constructor.name.toLowerCase());
        const dataset = bigquery_client_1.BigQueryClient._bigQuery.dataset(registry.getDataset()), tableName = this.getTableName(rc), table = dataset.table(tableName);
        rc.isDebug() && rc.debug(rc.getName(this), this.name, ' insert data to table', tableName, '[ Version ' + registry.getVersion() + ']');
        return table;
    }
    async insert(rc) {
        ...
        const clazz = this.constructor, traceId = clazz.name + ':' + 'BqInsert:' + Date.now(), ack = rc.startTraceSpan(traceId);
        try {
            const table = await clazz.getDataStoreTable(rc), bqNiData = this;
    ...
    async streamBulkInsert(rc, items) {
        const registry = bigquery_registry_1.BqRegistryManager.getRegistry(this.constructor.name.toLowerCase());
        ...
        const table = await this.getDataStoreTable(rc);
        ...
        try {
            ...
            if (error && error.name === 'PartialFailureError') {
                ...
                error.errors.forEach((element) => {
                    errorRowCount++;
                    let errorDesc = '';
                    element.errors.forEach((errorDescObject) => {
    ...
    async streamInsert(rc) {
        let err = this.fieldsError(rc);
        if (err) {
            rc.isWarn() && rc.warn(rc.getName(this), 'Data Sanity Failed. Not inserting the model.', err);
            return;
        }
        const clazz = this.constructor, traceId = clazz.name + ':' + 'streamInsert:' + Date.now(), ack = rc.startTraceSpan(traceId);
        try {
            const table = await clazz.getDataStoreTable(rc), bqNiData = this;
            rc.isDebug() && rc.debug(rc.getName(this), 'data : ', bqNiData);
            const res = await table.insert(bqNiData, this.getTableInsertOptions(rc));
    ...
    static async getTableData(rc, query, useLegacySql = false) {
        const options = {
            query: query,
            useLegacySql: useLegacySql // Use standard SQL syntax for queries.
        };
        const result = await bigquery_client_1.BigQueryClient._bigQuery.query(options);
        return result;
    }
    async bulkInsert(rc, items) {
        const registry = bigquery_registry_1.BqRegistryManager.getRegistry(this.constructor.name.toLowerCase());
        for (const item of items) {
            // check the fields sanity of all items
    ...
    static async listTables(rc, dsName) {
        const traceId = `BqListTables:${Date.now()}`, ack = rc.startTraceSpan(traceId);
        try {
            const dataset = bigquery_client_1.BigQueryClient._bigQuery.dataset(dsName), data = await dataset.getTables();
    ...
    static async deleteTable(rc, id, dsName) {
        const traceId = `BqDeleteTable:${Date.now()}`, ack = rc.startTraceSpan(traceId);
        try {
            const dataset = bigquery_client_1.BigQueryClient._bigQuery.dataset(dsName), table = dataset.table(id);
            await table.delete();
    ...
    async getDataset(rc, registry) {
        let dataset = await bigquery_client_1.BigQueryClient._bigQuery.dataset(registry.getDataset());
        if (!dataset.exists()) {
            ...
            const [createdDataset] = await bigquery_client_1.BigQueryClient._bigQuery.createDataset(registry.getDataset(), options);
            dataset = createdDataset;
        }
        return dataset;
    }
}
exports.BqBaseModel = BqBaseModel;
BqBaseModel.DATE_FORMAT = '%yyyy%%mm%%dd%';
//# sourceMappingURL=bq-base-model.js.map