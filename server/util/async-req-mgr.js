...
exports.AsyncReqManager = void 0;
const lo = require("lodash");
class AsyncReqManager {
    constructor(rc, name, maxParallelReqCount, load) {
        this.rc = rc;
        this.name = name;
        this.maxParallelReqCount = maxParallelReqCount;
        this.load = load;
        this.requests = [];
    ...
    async makeRequest(rc, asyncFunc, thisObj, ...funcArgs) {
        const now = Date.now(), len = AsyncReqManager.buff.length - 1, buff = AsyncReqManager.buff, randStr = [buff[lo.random(len)], buff[lo.random(len)], buff[lo.random(len)]].join(''), id = now + '-' + randStr + '-' + AsyncReqManager.k++;
        if (AsyncReqManager.k > 100000)
            AsyncReqManager.k = 0;
        let req = { id: id, rc: rc, reqTs: now, args: funcArgs, func: asyncFunc.bind(thisObj), resolve: null, reject: null };
        ...
        const startReq = this.canStartRequest(rc);
        if (!startReq) {
    ...
    canStartRequest(rc) {
        if (this.requests.length && (this.maxParallelReqCount <= 0 || (this.activeReqCount < this.maxParallelReqCount))) {
            // check load 
            if (this.load) {
                const dur = this.load.duration, now = Date.now();
    ...
    finish(req, startTime, resolvedValue, err) {
        this.activeReqCount--;
...
exports.AsyncReqManager = AsyncReqManager;
AsyncReqManager.k = 0;
AsyncReqManager.buff = 'ABCDEFGHIJKLKMNOPQRSTUVWXYZ';
//# sourceMappingURL=async-req-mgr.js.map